...
import type { Employee, Departament, EmployeeStatus, EmployeeRole } from '../types';
...
function EmployeesPage() {
  ...
  const [newDepartament, setNewDepartament] = useState<Departament>('Tecnologia');
  ...
  const filteredEmployees = employees.filter(emp => {
    ...
    const matchesDepartament = !selectedDepartament || emp.Departament === selectedDepartament;
    const matchesStatus = !selectedStatus || emp.status === selectedStatus;

    return matchesSearch && matchesDepartament && matchesStatus;
  });
  ...
  const handleAddEmployee = useCallback(() => {
    ...
    setNewAvatarUrl('');
    setShowForm(false);
  }, [newName, newEmail, newPosition, newDepartament, newSalary, newHireDate, newStatus, newRole, newPhone, newAvatarUrl]);

  // Listas y diccionarios de apoyo
  const Departaments: Departament[] = ['Tecnologia', 'Recursos Humanos', 'Finanzas', 'Operaciones', 'Ventas'];
          ...
          <div style={{
            ...
            <FormField label="Departamento *">
              <select
                ...
                onChange={(e) => setNewDepartament(e.target.value as Departament)}
                style={formFieldStyle}
              >
      ...
      <div style={{
        ...
        <FormField label="Departamento" style={{ minWidth: '180px' }}>
          <select
            ...
            onChange={(e) => setSelectedDepartament(e.target.value as Departament | '')}
            style={{
              padding: '8px 12px', 
              border: '1px solid #cbd5e1',
              borderRadius: '6px', 
        ...
        {(search || selectedDepartament || selectedStatus) && (
          <button
            onClick={() => { 
              setSearch(''); 
